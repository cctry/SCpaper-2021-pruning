    ...
    def train(self, model_path: Optional[str] = None):
        ...
        if self.args.rew:
            ...
            for i in range(len(layers)):
                ...
                elif (self.args.sparsity_type == "row"):
                    rew_layers.append(torch.abs(1 / (torch.norm(conv_layer.data, dim=1) + eps)))
...
#####################################################
                ...
                elif (self.args.sparsity_type == "whole_block_padding_balanced"):  # -libn
                    shape = conv_layer.shape
                    conv = conv_layer.reshape(shape[0], -1)
                    shape = conv.shape
                    ...
                    if conv.shape[1] % block_col_width != 0 or conv.shape[0] % block_row_width != 0:
                        ...
                        conv = np.concatenate((conv, np.zeros((conv.shape[0], padding_width)).astype(conv.dtype)),
                                              axis=1)
                    ...
                    whole_block_norms = torch.Tensor(0).float().cuda()
                    ...
                    for i in range(block_row_division):
                        ...
                        block_norms = torch.chunk(block_row_norms.reshape(1, len(block_row_norms)), block_col_division,
                                                  dim=1)
                        # Concatenate the row l2 norms:
                        block_norms = torch.cat(block_norms, 0)
                        block_norms = torch.norm(block_norms, dim=1)
                        # print("data type: ", whole_block_norms.dtype, block_norms.dtype)
                        whole_block_norms = torch.cat((whole_block_norms, block_norms), 0)

                    rew_layers.append(1 / (whole_block_norms + eps))
                elif (self.args.sparsity_type == "whole_block_padding"):  # -libn
                    shape = conv_layer.shape
                    conv = conv_layer.reshape(shape[0], -1)
                    shape = conv.shape
                    ...
                    block_norms = torch.chunk(block_row_norms.reshape(1, len(block_row_norms)), block_row_division,
                                              dim=1)
                    # Concatenate the row l2 norms:
                    block_norms = torch.cat(block_norms, 0)
                    block_norms = torch.norm(block_norms, dim=1)
                ...
                if self.args.rew:
                    ...
                    if step == 0 and epoch in milestone:
                        ...
                        for j in range(len(layers)):
                            if (self.args.sparsity_type == "irregular"):
                                ...
                                rew_layers[j] = torch.abs((1 / (torch.norm(layers[j].data, dim=1) + eps)))
                            elif (self.args.sparsity_type == "filter"):
                            ...
                            elif (self.args.sparsity_type == "whole_block_padding_balanced"):
                                shape = conv_layer.shape
                                conv = conv_layer.reshape(shape[0], -1)
                                shape = conv.shape
                                ...
                                if conv.shape[1] % block_col_width != 0 or conv.shape[0] % block_row_width != 0:
                                    padding_height = (block_row_width - conv.shape[0] % block_row_width) % (
                                        block_row_width)
                                    padding_width = (block_col_width - conv.shape[1] % block_col_width) % (
                                        block_col_width)
                                    conv = conv.cpu().detach().numpy()
                                    conv = np.concatenate((conv, np.zeros((padding_height, conv.shape[1]))), axis=0)
                                    conv = np.concatenate(
                                        (conv, np.zeros((conv.shape[0], padding_width)).astype(conv.dtype)),
                                        axis=1)
                                ...
                                for i in range(block_row_division):
                                    ...
                                    convfrag = torch.cat(convfrag, 0)
                                    block_row_norms = torch.norm(convfrag, dim=1)
                                    block_norms = torch.chunk(block_row_norms.reshape(1, len(block_row_norms)),
                                                              block_col_division,
                                                              dim=1)
                                    # Concatenate the row l2 norms:
                                    block_norms = torch.cat(block_norms, 0)
                                    ...
                                    # print("data type: ", whole_block_norms.dtype, block_norms.dtype)
                                    whole_block_norms = torch.cat((whole_block_norms, block_norms), 0)
                                rew_layers[j] = (1 / (whole_block_norms + eps))
                            elif (self.args.sparsity_type == "whole_block_padding"):  # -libn
                                shape = conv_layer.shape
                                conv = conv_layer.reshape(shape[0], -1)
                                shape = conv.shape
                    ...
                    for j in range(len(layers)):
                        ...
                        if (self.args.sparsity_type == "irregular"):
                            ...
                            l1_loss = l1_loss + penalty_factors[layers_names[j]] * torch.sum(rew * torch.norm(conv_layer, dim=1))
                        elif (self.args.sparsity_type == "filter"):
                        ...
                        # block-filter:
                            ...
                            l1_loss = l1_loss + penalty_factors[layers_names[j]] * torch.sum(rew * whole_block_norms)
                        elif (self.args.sparsity_type == "whole_block_padding"):  # -libn
                            shape = conv_layer.shape
                            conv = conv_layer.reshape(shape[0], -1)
                            shape = conv.shape
