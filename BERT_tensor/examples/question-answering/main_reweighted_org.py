...
parser.add_argument('--cross-x', type=int, default=9,
                    help='the crossbar x number, set to 4 to disable')
parser.add_argument('--cross-f', type=int, default=4,
                    help='the crossbar filter number, set to 1 to disable')
parser.add_argument('--same-size', action='store_true', default=False,
                    help='make every block the same size')
...
def reweighted_training(criterion, optimizer, scheduler):
    ...
    for i in range(len(layers)):
        ...
        if args.sparsity_type == "irregular":
            ...
            rew_layers.append(torch.abs(1 / (torch.norm(conv_layer.data, dim=[2, 3]) + eps)))
            ...
            if conv.shape[1]%args.cross_x != 0 :
                print("the layer size is not divisible",conv.shape[0], conv.shape[1], args.cross_f, args.cross_x)
...
def masked_retrain(criterion, optimizer, scheduler):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if prec1 > max(best_prec1):
            ...
            torch.save(model.state_dict(), "./model_retrained/cifar10_{}{}_retrained_acc_{:.3f}_{}_{}.pt".format(
            ...
            if len(best_prec1) > 1:
                os.remove("./model_retrained/cifar10_{}{}_retrained_acc_{:.3f}_{}_{}.pt".format(
...
def train(train_loader, criterion, optimizer, scheduler, epoch, args, layers, rew_layers, eps):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.rew:
            ...
            if i == 0 and epoch - 1 in rew_milestone:
                ...
                for j in range(len(layers)):
                    if args.sparsity_type == "irregular":
                        ...
                        rew_layers[j] = torch.abs((1 / (torch.norm(layers[j].data, dim=[2, 3]) + eps)))
                    elif args.sparsity_type == "filter":
                        ...
                        for j in range(len(convfrag)):
                            if mat is None:
                                ...
                                mat = torch.cat((mat, convfrag[j]), 0)
                        rew_layers[j] = (1 / (torch.norm(mat.data, dim=1) + eps))
                ...
                if args.sparsity_type == "irregular":
                    ...
                    for j in range(len(convfrag)):
                        if mat is None:
                            ...
                            mat = torch.cat((mat, convfrag[j]), 0)
                    l1_loss = l1_loss + 1e-3 * torch.sum(rew * torch.norm(mat, dim=1))
            ...
            # ce_loss = l1_loss   #test if the grad is not 0 in reweighted part
        ...
        #     if(len(W.size()) == 4) :
        #         print(W.grad)    # test if the grad is not 0 in reweighted part
